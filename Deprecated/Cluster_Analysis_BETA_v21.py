...
import sys, time, os, traceback, random, time, ConfigParser, csv, math
...
from ij.plugin import ChannelSplitter, ImageCalculator, RGBStackMerge, ZProjector, Duplicator, StackEditor, Concatenator, RoiEnlarger
...
#	dir_path = os.path.realpath('__file__')
	...
	#dir_path = "./Fiji.app"
	dir_path = os.path.dirname(os.path.realpath('__file__'))
	dir_path = os.path.join(dir_path, "ImageJ2")
...
class db_interface(object):
	...
	def writeCSV(self):
		...
		self.dbConn = self.getConnection()
		tables = ["Particle_Analysis_Table", "PA_Measurement_Tables", "Coloc_Analysis_Table", "Coloc_Measurement_Tables"]
		for t in tables:
...
class Selection(object):
	...
	def selectAreaAuto(self):
		...
		if not headless:
			...
			gd.addMessage("_________________________________________________________________________________")
			gd.addMessage("Do you want to perform a dendritig segment analysis? (set options to 0 if not)")
	...
	def particleAnalysis(self, imp):
		...
		msInt = ms.AREA
		rt = ResultsTable()
		pa = ParticleAnalyzer(options, msInt, rt, math.pi*cal.getRawX(math.sqrt(self.sizea)/math.pi)**2, math.pi*cal.getRawX(math.sqrt(self.sizeb)/math.pi)**2, Double(self.circa), Double(self.circb))
...
class ParticleAnalyser(object):
	...
	def analyzePA(self, imp, roi, inorout="", paString=""):
		...
		if not paString:
			pa = ParticleAnalyzer(options, measurements, rt, cal.getRawX(math.sqrt(self.channel.lowerSize))**2, cal.getRawX(math.sqrt(self.channel.higherSize))**2, self.channel.circ1, self.channel.circ2)
			print "Corrected: ", cal.getRawX(math.sqrt(self.channel.lowerSize))**2, cal.getRawX(math.sqrt(self.channel.higherSize))**2, cal.getRawX(self.channel.lowerSize**0.5)**2, cal.getRawX(self.channel.higherSize**0.5)**2
			print "Raw: ", self.channel.lowerSize, self.channel.higherSize, cal.getRawX(self.channel.lowerSize), cal.getRawX(self.channel.higherSize)
		else:
			pa = ParticleAnalyzer(options, measurements, rt, cal.getRawX(math.sqrt(paString[0]))**2, cal.getRawX(math.sqrt(paString[1]))**2, paString[2], paString[3])
		pa.setHideOutputImage(True)
		imp.setRoi(roi)
		ip = imp.getProcessor()
		ip.setRoi(roi)
		...
		for m in self.channel.method:
			...
			if self.show:
				if self.new_roi:
					...
					flat2 = flat.flatten()
					flat2.setTitle("Binary-%s-%s-%s" % (self.channel.channel_name, self.roi.getName(), m))
					flat2.show()
					imp2.close()
				else:
			...
			self.new_roi.setName("Binary-%s-%s-%s" % (self.channel.channel_name, self.roi.getName(), m))
			return self.new_roi
	...
	def coloc(self, sub2, channel2, index):
		...
		if self.new_roi:
			if self.channel.pa_inside:
				...
				#self.roisInside = roi
				roi.setName("Inside_" + self.channel.channel_name + "_" + channel2.channel_name + "_" + m + "_" + str(index))
			...
			if self.channel.pa_outside:
				...
				roi.setName("Outside_" + self.channel.channel_name + "_" + channel2.channel_name + "_" + m + "_" + str(index))
				if self.show:
