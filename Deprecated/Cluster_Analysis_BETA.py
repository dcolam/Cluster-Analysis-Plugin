...
import sys, time, os, traceback, random, time, ConfigParser, csv
...
class Dialoger(object):
	...
	def getParticleAnalyzerOptions(self, channel_number, coloc=''):
		...
		if not headless:
			...
			if channel_number == 0:
				gd.addNumericField("Lower Particle Size:", paSizeA1, 0) 
				gd.addNumericField("Higher Particle Size:", paSizeB1, 0) 

			else:
				gd.addNumericField("Lower Particle Size:", paSizeA2, 3) 
				gd.addNumericField("Higher Particle Size:", paSizeB2, 0) 
...
class ParticleAnalyser(object):
	...
	def analyzePA(self, imp):
		rt = ResultsTable()
		options = ParticleAnalyzer.SHOW_MASKS
		measurements = ms.AREA
		pa = ParticleAnalyzer(options, measurements, rt, self.channel.lowerSize, self.channel.higherSize, self.channel.circ1, self.channel.circ2)
	...
	def makeBinary(self, roi):
		...
		for m in self.channel.method:
			...
			#roisIndex = []
			#for r in rois:
		#		roisIndex.append(RM.getRoiIndex(r))
			#RM.setSelectedIndexes(roisIndex)
	...
	def coloc(self, sub2, channel2, index):
		...
		def flatShow(colocMask, inorout):
	...
	def colocPA(self, inorout, binary, paString):
		...
		tp = IJ.getTextPanel()
		tp_string = tp.getText()
		return colocMask, tp_string, area
